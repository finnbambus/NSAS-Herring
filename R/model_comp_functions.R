...
cross_valid <- function(dataset) {
  ...
  for(i in 1:nrow(dataset)){
    ...
    # 1. Linear model
    tryCatch({
      m1 <- lm(R ~ SSB_lag, data = train_data, na.action = na.exclude)
    ...
    # 2. Beverton-Holt
    tryCatch({
      if(any(train_data$R <= 0, na.rm = TRUE) || any(train_data$SSB_lag <= 0, na.rm = TRUE)) {
        cat("Warning: Non-positive values detected for Beverton-Holt model in fold", i, ". Skipping.\n")
      } else {
        svR <- tryCatch(srStarts(R ~ SSB_lag, data = train_data, type = "BevertonHolt"),
                        error = function(e) {
        ...
        if (!is.null(svR)) {
          ...
          srR_beverton <- nls(log(R) ~ log(bh(SSB_lag, a, b)),
                              data = train_data,
                              start = svR,
    ...
    tryCatch({
      if(any(train_data$R <= 0, na.rm = TRUE) || any(train_data$SSB_lag <= 0, na.rm = TRUE)) {
        ...
        svR <- tryCatch(srStarts(R ~ SSB_lag, data = train_data, type = "Ricker"),
                        error = function(e) {
        ...
        if (!is.null(svR)) {
          ...
          srR_ricker <- nls(log(R) ~ log(rckr(SSB_lag, a, b)),
                            data = train_data,
                            start = svR,
                            control = nls.control(maxiter = 50, minFactor = 1/2048),
                            na.action = na.exclude)
    ...
    tryCatch({
      base_lm <- lm(R ~ SSB_lag, data = train_data, na.action = na.exclude)
      seg <- segmented::segmented(base_lm,
                                  seg.Z = ~SSB_lag,
                                  psi = mean(train_data$SSB_lag, na.rm = TRUE))
    ...
    # 5. Segmented log
    tryCatch({
      if("R_log" %in% names(train_data) && "SSB_lag_log" %in% names(train_data) &&
         all(is.finite(train_data$R_log)) && all(is.finite(train_data$SSB_lag_log))) {
        base_lm_log <- lm(R_log ~ SSB_lag_log, data = train_data, na.action = na.exclude)
        seg_log <- segmented::segmented(base_lm_log,
                                        seg.Z = ~ SSB_lag_log,
                                        psi = mean(train_data$SSB_lag_log, na.rm = TRUE))
        
        if("R_log" %in% names(test_data) && "SSB_lag_log" %in% names(test_data)) {
    ...
    formula_str <- "R ~ SSB_lag"
    seg_formula_str <- "~SSB_lag"
    mean_ssb <- mean(train_data$SSB_lag, na.rm = TRUE)
    ...
    tryCatch({
      bpts <- strucchange::breakpoints(R ~ SSB_lag, data = train_data, h = 0.15)
        ...
        if (opt_breaks > 0) {
          ...
          if(!is.null(bp_final$breakpoints) && !any(is.na(bp_final$breakpoints))) {
            ...
            if(length(levels(train_bp_factor)) > 1) {
              strucc_model <- lm(R ~ train_bp_factor * SSB_lag, data = train_data)
              ...
              # Test prediction
              bp_values <- train_data$SSB_lag[bp_final$breakpoints]
              test_segment <- cut(test_data$SSB_lag, 
