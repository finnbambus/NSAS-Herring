...
SSB_LAI <- SSB_full %>%
  ...
  dplyr::select(year, component, LAI_perc., SSB_component, l_bnd_component, u_bnd_component) %>%
  drop_na()
```
...
plot_SSB_component <- ggplot(SSB_LAI, aes(x = year, y = (SSB_component / 1000000))) +
...
plot_SSB_component_b <- ggplot(SSB_LAI, aes(x = year, y = SSB_component / 1e6, color = component, fill = component)) +
...
save(SSB_full, file = file.path("data/", "SSB_full.Rdata"))
save(SSB_LAI, file = file.path("data/", "SSB_LAI.Rdata"))

### ICES data
save(ICES_data, file = file.path("data/", "ICES_data.Rdata"))
